...
* `*_arxos.bin` (Example: shakti_c_newp_arxos.bin)
* `qemu.bat`
...
Update the path to the `*_arxos.bin` image (for example: `shakti_c_newp_arxos.bin`).  
The `*_arxos.bin` image contains both the ARX RTOS and the application software built together.

Example:

```text
set ARXIMAGE=C:/<Your_PC_Path>/arx-rtos-main/arxos/arch/riscv/rv64/rv64gc/shakti_c/shakti_c_newp_arxos.bin
```
---
